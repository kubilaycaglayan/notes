...
def redactor
  ...
  words = text.split(' ')
  dodge_words_upcased = dodge_words.map(&:upcase)
  redacted_text = ''
  words.each do |word|
    redacted_text << if dodge_words.include?(word) || dodge_words_upcased.include?(word.upcase)
                       '**** '
                     else
                       "#{word} "
                     end
...
def multiple_answer
  ...
  while quantity_not_integer
    ...
    quantity_not_integer = false if (1..4).include?(quantity)
  end
  counter = quantity
...
def word_counter
  ...
  frequencies = frequencies.sort_by do |_word, frequence|
    frequence
  end
  frequencies.reverse!
